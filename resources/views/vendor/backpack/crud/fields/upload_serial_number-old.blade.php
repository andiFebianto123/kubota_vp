...
<div class="container-repeatable-elements">
  ...
  <button type="button" class="btn btn-link btn-sm ml-1 add-repeatable-element-button">+ {{ $field['new_item_label'] ?? trans('backpack::crud.new_item') }}</button>
  <button class="btn btn-sm btn-link pull-right" type="button" data-toggle="modal" data-target="#modalUploadSerialNumber" > <i class="la la-file-excel"></i> Upload Excel</button>
                    ...
                    if (response.status) {
                        messageStatusGeneral("#"+formId, response.message, 'success')
                            ...
                            $.each( response.datas, function( key, row ) {
                                $("[data-repeatable-input-name='sn_childs[]']:eq("+key+")").val(row.serial_number)
                                newRepeatableElement(container, field_group_clone)
                            })
